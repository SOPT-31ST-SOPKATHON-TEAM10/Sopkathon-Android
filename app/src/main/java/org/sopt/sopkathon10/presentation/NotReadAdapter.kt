...
class NotReadAdapter : RecyclerView.Adapter<NotReadAdapter.NotReadViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NotReadAdapter.NotReadViewHolder {
        val binding =
            ItemMessageListBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return NotReadAdapter.NotReadViewHolder(binding)
    }

    override fun onBindViewHolder(holder: NotReadAdapter.NotReadViewHolder, position: Int) {
        entire_items?.let {
